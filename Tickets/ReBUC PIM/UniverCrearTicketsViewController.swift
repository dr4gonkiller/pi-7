...
class UniverCrearTicketsViewController: UIViewController {
    ...
    @IBAction func crearTicket(_ sender: UIButton) {
        ...
        //Guardar el ticket
        let registrarTicket = self.ticketsTabla.insert(self.idUsuarioExp <- self.idUsuario!, self.fechaTicketExp <- self.fechaActual!, self.consultaExp <- self.consultaTextField.text!, estatusExp <- "Nuevo", calificacionExp <- 0)
